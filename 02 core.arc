...
#|(mac rlet (name var x . body)
  #`(rwith name (var x) . body))

(mac alet (var x . body)
  #`(rlet 'self var x . body))|#

(mac alet (var x . body)
  #`(awith (var x) . body))


#|(def uniq ((o name 'g) (o num))
  (let num (if num num (let num (ac-uniq-counter)
                         (ac-uniq-counter (racket-+ (ac-uniq-counter) 1))
                         num))
    (racket-string->uninterned-symbol
      (racket-string-append (ac-string1 name)
                            (ac-string1 num)))))|#
